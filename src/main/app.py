...
class App:
    ...
    def settings_menu(self):
        ...
        while menu_open:
            ...
            print(
                ...
                f'[4] Turn {"off" if self.game.settings["keyboard"] else "on"} keyboard display\n'
                "\n[S] Save settings\n"
                "[C] Cancel"
            )
            ...
            if command == "4":
                self.game.settings["keyboard"] = not self.game.settings["keyboard"]

            elif command.casefold() in ("s", "c"):
                if (
                    old_settings != self.game.settings
                ):  # if settings were changed then confirmation is required
                    while True:
                        cc()
                        ...
                        if yes_or_no.casefold() == "y":
                            if command.casefold() == "c":
                                self.game.settings = (
                                    old_settings  # restores previous settings
                                )
    ...
    def highscore_menu(self):
        while True:
            ...
            if command == "1":
                self.game.highscore_list.print(
                    self.game.settings["table_style"], cutoff=10
                )
                ...
                self.game.highscore_list.print(
                    self.game.settings["table_style"], search=input()
                )
    ...
    def help_menu(self):
        examples = [
            self.game.check_guess("table", answer="lobby"),
            self.game.check_guess("heal", answer="healthy"),
        ]
