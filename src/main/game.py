...
class Game:
    ...
    def run(self):
        ...
        keyboard = Keyboard(self.color_letter)
        ...
        while guessing:
            ...
            if guess == self.answer:
                self.guesses.append(self.check_guess(self.answer, keyboard))
                self.win(time.time() - start_time)
                guessing = False

            else:
                self.guesses.append(self.check_guess(guess, keyboard))
    ...
    def win(self, guess_time: float):
        ...
        highscore_place = self.highscore_list.get_place(
            ["", len(self.guesses), self.answer, final_time]
        )
        ...
        if highscore_place < 100:  # achieved top 100
            ...
            self.highscore_list.insert(
                highscore_place, [name, len(self.guesses), self.answer, final_time]
            )
        ...
        if highscore_place < 10:  # prints top 10
            ...
            self.highscore_list.print(
                self.settings["table_style"],
                cutoff=10,
                bottom_entry=[
    ...
    def color_letter(self, letter: str, color: str) -> str:
        ...
        if color == "green":
            return (
                color_text(f" {letter} ", background="lime")
                if not self.settings["colorblind"]
                else color_text(f" {letter} ", invert=True)
            )

        elif color == "yellow":
            return (
                color_text(f" {letter} ", background="yellow")
                if not self.settings["colorblind"]
                else color_text(f" {letter} ", underline=True, bold=True)
            )
            ...
            return (
                color_text(f"{letter}", background="red")
                if not self.settings["colorblind"]
                else f"{letter}"
            )
